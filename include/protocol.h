...
typedef enum {
  ...
  CMD_NACK,
  /* Device send ping to init or check for connection integrity. Should be answered with CMD_PONG if no trans pending, or other host cmd otherwise. */
  CMD_PING,
  /* Answer for CMD_PING if no trans pending. */
  CMD_PONG
} command_t;
